...
class VocoderDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not len(wav) >= mel.shape[1] * hp.hop_length:
            wav = np.pad(wav, (0, mel.shape[1] * hp.hop_length), mode='constant')
            print(f"length: {len(wav)}, shape: {mel.shape[1] * hp.hop_length}")
